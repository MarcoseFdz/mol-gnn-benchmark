...
This project provides a robust benchmarking framework for **Graph Neural Networks (GNNs)** applied to molecular property prediction. Specifically, it targets the **BACE (Beta-secretase 1)** dataset, a critical task in drug discovery for Alzheimer's disease.
...
By leveraging advanced graph data augmentation and modern deep learning practices, this repository demonstrates how GNNs can effectively capture structural molecular information directly from SMILES strings.
...
Below is the comprehensive evaluation dashboard generated by `evaluate.py`. It includes ROC/PR curves, confusion matrices, and a radar chart of key metrics.
...
### Installation
```bash
pip install tensorflow rdkit matplotlib scikit-learn pandas numpy
```
